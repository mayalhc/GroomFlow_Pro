...
* **더 빠른 Children — 뷰포트 부하 대폭 감소**
  * 이번 릴리스에서 실시간 Children 엔진의 성능이 크게 향상되었습니다. Children이 활성화된 상태에서의 스컬핑, 콤빙, 시뮬레이션이 특히 Child Count가 높거나 가이드 커브가 많은 경우에도 훨씬 가볍게 느껴집니다.
  * 이는 내부 엔진 개선 사항으로, 설정 변경이나 추가 작업은 필요 없습니다. 기존과 동일하게 Build Children을 사용하면 향상된 반응성을 바로 체감할 수 있습니다.
<br>
<br>
* **Live 엔진 — Live가 OFF일 때도 패시브 팔로우 유지**
  * 이번 릴리스에서 가장 큰 변화: **Live** 토글이 꺼진 상태에서도 Children이 가이드 커브를 계속 따라갑니다.
  * Blender의 기본 Hair Dynamics를 가이드 커브에 켜거나, 애니메이션이 적용된 캐릭터를 재생하면 Children이 Live 엔진 없이도 함께 움직입니다.
  * 즉, **Live ON**은 스컬핑이나 콤빙 작업 중 클럼프/트위스트 디테일을 실시간으로 재계산해야 할 때만 사용하면 됩니다. 형태 작업이 끝나면 **Live OFF**로 전환하세요. 그루밍이 멈추지 않고 가이드에 적용된 시뮬레이션이나 애니메이션을 거의 추가 비용 없이 계속 따라갑니다.
  * 자세한 내용은 아래 **섹션 9. 실시간 심플 Children**을 참조하세요.
...
* **Target Base Scale (타겟 기본 스케일)**
  * 생성된 모든 헤어 가이드 커브의 전역 기본 스케일 계수를 설정합니다.
  * 씬 단위 또는 캐릭터 메쉬 스케일 차이로 인해 헤어가 너무 작거나 크게 보이는 현상을 방지합니다.
  * 기본값 1.0에서 이 값을 조정하여 모든 가이드 커브의 스케일을 균일하게 제어합니다.
...
Children 시스템은 각 가이드 커브 주변에 짧고 자연스럽게 분포된 차일드 스트랜드 군집을 생성합니다. v1.5.0부터 Children은 두 가지 방식으로 가이드와 동기화를 유지합니다. 항상 작동하는 경량 팔로우와 선택적 정밀 실시간 재계산을 통해, 항상 최대 성능 비용을 지불하지 않고도 실시간 피드백을 얻을 수 있습니다.
...
* **Spread Along Guide** — Root Spread가 0보다 클 때, 평평한 디스크(0.0)와 가이드 방향을 따르는 분포(1.0) 사이를 블렌딩합니다.
...
## 13. 언리얼 엔진 파이프라인 및 Expert Synergy 워크플로우

에셋 데이터 블록을 마무리하고 시네마틱 익스포트 통합을 위해 그룸을 준비합니다.
<br>
<br>
* **전역 스케일 보정 (Target Base Scale 컨트롤)**
  * Blender와 언리얼 엔진은 FBX/Groom 월드 스케일 행렬을 완전히 다르게 처리합니다.
  * 익스포트 시 임포트된 헤어스타일이 거대한 시트로 폭발하거나 미세한 핀으로 축소되는 경우가 자주 발생합니다.
  * `절차적 사전 베이킹 보정`:
    * 원시 Blender 단위를 신뢰할 수 없이 변경하거나 에셋 익스포트 중 행렬 아티팩트의 위험을 감수하는 대신, **Target Base Scale** 슬라이더를 조정하여 그룸을 동적으로 보정합니다.
  * `완벽한 파이프라인 타겟`:
    * 표준 Blender 렌더링 설정에서는 `1.00`으로 유지합니다.
    * 최종 시뮬레이션 익스포트를 실행하기 전에 타겟 엔진 스케일링 계수에 정확하게 설정합니다.
    * 핵심 메쉬 스케일을 변경하지 않고 언리얼 엔진 5.7+ 새 Dataflow 시스템으로의 절대 변환 동기화를 강제합니다.
<br>
<br>
